...
export async function getAuthenticatedUser() {
  try {
    ...
    // Fallback to Firebase cookies for backward compatibility
    if (!userCookie) {
      userCookie = cookieStore.get('firebase-user');
      tokenCookie = cookieStore.get('firebase-token');
      isCognito = false;
      console.log('[auth-server] Falling back to Firebase cookies:', {
        hasFirebaseUser: !!userCookie,
        hasFirebaseToken: !!tokenCookie
      });
    }
    ...
    console.log(`[auth-server] Authenticating user: ${userId} (${isCognito ? 'Cognito' : 'Firebase'})`);
    ...
    try {
      ...
      // Try to find user by firebaseUserId (which stores both Firebase and Cognito IDs)
